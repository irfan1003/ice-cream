...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Purchase Orders Masuk (Admin)</h1>
                <p class="text-sm text-slate-500 mt-1">Daftar Purchase Orders yang menunggu verifikasi Admin.</p>
            </div>

            <div class="flex items-center gap-3">
                <!-- Export Button -->
                <a href="{{ route('admin.po.export', ['status' => 'pending_admin']) }}"
                ...
                <!-- Search Form -->
                <form action="{{ route('admin.po.index') }}" method="GET" class="relative group">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        @forelse($purchaseOrders as $po)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4 font-medium text-slate-900">#{{ $po->po_number }}</td>
                                <td class="px-6 py-4 text-slate-600">
                                    {{ \Carbon\Carbon::parse($po->po_date)->format('d M Y') }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="font-medium text-slate-900">{{ $po->customer->customer_name }}</div>
                                    <div class="text-[11px] text-slate-400">{{ $po->customer->phone }}</div>
                                </td>
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <p>Tidak ada purchase orders masuk.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
                            ...
                            <table class="w-full text-left text-sm border-collapse">
                                ...
                                <tfoot class="bg-slate-50/50">
                                    ...
                                    <tr class="font-bold text-slate-900 bg-slate-50">
                                        ...
                                        <td class="px-4 py-3 text-right text-emerald-600 text-base" id="modalGrandTotal">
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
        ...
        <script>
            function approvePO(poId, poNumber) {
                Swal.fire({
                    ...
                    text: `PO ${poNumber} akan diverifikasi dan diteruskan ke Direktur untuk persetujuan akhir.`,
                    icon: 'question',
                    ...
                    if (result.isConfirmed) {
                        fetch(`{{ url('/admin/purchase-orders') }}/${poId}/approve`, {
                            method: 'POST',
                            headers: {
            ...
            function showPODetails(po) {
                ...
                // Set basic info
                document.getElementById('modalPONumber').innerText = `Detail Purchase Order #${po.po_number}`;
                ...
                document.getElementById('modalCustomerPhone').innerText = po.customer?.phone || '-';
                ...
                document.getElementById('modalSubtotal').innerText = `Rp ${new Intl.NumberFormat('id-ID').format(po.subtotal)}`;
                document.getElementById('modalDiscount').innerText = `- Rp ${new Intl.NumberFormat('id-ID').format(po.discount_total || 0)}`;
