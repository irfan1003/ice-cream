...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
            <div class="flex items-center gap-4">
                <div
                    class="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center text-amber-600 group-hover:scale-110 transition-transform">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                    ...
                    $filteredOrders = $orders->filter(function ($order) use ($table) {
                        ...
                        if ($table['status'] === 'approved') {
                            return $order->status === 'approved' &&
                                (!$order->delivery ||
                                    !in_array($order->delivery->delivery_status, ['shipped', 'delivered']));
                        }
                                ...
                                @forelse ($filteredOrders as $order)
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <p class="text-slate-400 text-[11px]">Sales: {{ $order->sales->name ?? '-' }}
                                            </p>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            @php
                                                ...
                                                if (
                                                    $order->delivery &&
                                                    in_array($order->delivery->delivery_status, [
                                                        'shipped',
                                                        'delivered',
                                                    ])
                                                ) {
                                                ...
                                                $status = $statusConfig[$displayStatus] ?? [
                                                    'label' => str_replace('_', ' ', $displayStatus),
                                                    'class' => 'bg-slate-50 text-slate-600 border-slate-100',
                                                ];
                                            @endphp
                                            <span
                                                class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border {{ $status['class'] }}">
                                                {{ $status['label'] }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <button
                                                    onclick="showOrderDetail({{ json_encode($order->load('orderDetail.product')) }})"
                                                ...
                                                @if (
                                                    (in_array($order->status, ['approved', 'completed']) ||
    ...
    <div id="orderDetailModal" class="fixed inset-0 z-[60] hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onclick="closeOrderModal()">
            </div>
            <div
                        ...
                        <div class="border border-slate-100 rounded-xl overflow-hidden">
                            <table class="w-full text-left text-xs border-collapse">
                                ...
                                <tfoot class="bg-slate-50/30">
                                    <tr class="font-bold text-slate-900">
                                        <td colspan="3" class="px-4 py-4 text-right text-xs">GRAND TOTAL</td>
                                        <td class="px-4 py-4 text-right text-brand-pink text-sm" id="modalPOGrandTotal">
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function showOrderDetail(order) {
                ...
                document.getElementById('modalOrderGrandTotal').innerText =
            ...
            function showPODetails(po) {
                ...
                document.getElementById('modalPOGrandTotal').innerText =
