...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Verifikasi Pesanan (Direktur)</h1>
                <p class="text-sm text-slate-500 mt-1">Pesanan yang menunggu persetujuan akhir Anda.</p>
            </div>

            <!-- Search Form -->
            <form action="{{ route('direktur.verification.orders') }}" method="GET" class="relative group">
                        ...
                        @forelse($orders as $order)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-[11px] text-slate-500">{{ $order->customer->zone->zone_name ?? '-' }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <a href="{{ route('direktur.invoice.preview', $order->id_order) }}" target="_blank"
                                        ...
                                        <button
                                            onclick="reviseOrder({{ $order->id_order }}, '#{{ $order->order_number }}')"
                                        ...
                                        <button
                                            onclick="approveOrder({{ $order->id_order }}, '#{{ $order->order_number }}')"
    ...
    <!-- Detail Modal (Reuse same structure) -->
    ...
    @push('scripts')
        ...
        <script>
            function reviseOrder(orderId, orderNumber) {
                Swal.fire({
                    title: 'Revisi Pesanan?',
                    text: `Pesanan ${orderNumber} akan dikembalikan untuk direvisi.`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        processAction(`{{ url('/direktur/verification-orders') }}/${orderId}/revise`, 'POST');
                    }
                });
            }
            ...
            function approveOrder(orderId, orderNumber) {
                Swal.fire({
                    ...
                    text: `Menyetujui pesanan ${orderNumber} akan menggenerate Faktur PDF secara otomatis.`,
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            title: 'Memproses...',
                            text: 'Sedang menggenerate Faktur PDF...',
                            allowOutsideClick: false,
                            didOpen: () => {
                                Swal.showLoading();
                            }
                        });
                        processAction(`{{ url('/direktur/verification-orders') }}/${orderId}/approve`, 'POST');
                    }
                });
            }

            function processAction(url, method) {
                fetch(url, {
                        method: method,
                        headers: {
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Terjadi kesalahan sistem saat memproses tindakan.',
                            customClass: {
                                popup: 'rounded-2xl'
                            }
                        });
            ...
            function showOrderDetails(order) {
                ...
                document.getElementById('modalItems').innerHTML = itemsHtml ||
                    '<tr><td colspan="4" class="px-4 py-8 text-center text-slate-400 italic">Tidak ada item.</td></tr>';
                ...
                document.getElementById('modalDiscount').innerText =
                    `- Rp ${new Intl.NumberFormat('id-ID').format(order.discount_total || 0)}`;
