    ...
    <div x-data="purchaseCartSystem()"
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-0 bg-white">
            ...
            <div class="p-6 border-b border-slate-100 space-y-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Katalog Produk (Purchase Order)</h1>
                        <p class="text-sm text-slate-500">Pilih produk untuk membuat Purchase Order.</p>
                    </div>
                    ...
                    <template x-for="product in filteredProducts" :key="product.id_product">
                        <div @click="addToCart(product)"
                            ...
                            <div class="relative aspect-square rounded-[1.5rem] bg-slate-100 overflow-hidden mb-4 shadow-inner">
                                <img :src="product.image ? '{{ Storage::url('') }}/' + product.image.replace(/^\//, '') : 'https://images.unsplash.com/photo-1501443762994-82bd5dabb892?q=80&w=400&auto=format&fit=crop'"
                                    alt="Produk"
        ...
        <div id="cartPanel"
            ...
            <div class="px-6 py-4 flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-black text-slate-900 tracking-tight">Daftar Purchase Order</h2>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button @click="showCart = false" class="lg:hidden p-2.5 text-slate-400 hover:bg-slate-100 rounded-xl transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <template x-for="item in cart" :key="item.id_product">
                    <div class="flex items-center gap-4 group">
                        <div class="w-16 h-16 rounded-2xl bg-slate-50 overflow-hidden flex-shrink-0 border border-slate-100">
                            <img :src="item.image ? '{{ Storage::url('') }}/' + item.image.replace(/^\//, '') : 'https://images.unsplash.com/photo-1501443762994-82bd5dabb892?q=80&w=400&auto=format&fit=crop'"
                                alt="Produk"
            ...
            <div class="p-6 bg-slate-50/50 space-y-6">
                ...
                <button @click="openFinalModal()" :disabled="cart.length === 0"
                    ...
                    <div class="relative flex items-center justify-center px-6 py-4 bg-slate-900 text-white rounded-2xl transition-transform hover:scale-[1.02] active:scale-95">
                        <span class="font-black text-sm uppercase tracking-widest">Buat Purchase Order</span>
                    </div>
                </button>
            </div>
        </div>
        ...
        <div x-show="showFinalModal" 
            ...
            <div @click.away="showFinalModal = false"
                ...
                <div class="p-8 border-b border-slate-50 bg-slate-50/30">
                    <h3 class="text-xl font-black text-slate-900">Finalisasi Purchase Order</h3>
                    <p class="text-sm text-slate-500 mt-1">Lengkapi detail untuk Purchase Order ke pelanggan.</p>
                </div>
                            ...
                            <div class="p-5 bg-slate-50 rounded-3xl border border-slate-100 space-y-4">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl bg-white border border-slate-200 overflow-hidden">
                                            <img :src="item.image ? '{{ Storage::url('') }}/' + item.image.replace(/^\//, '') : 'https://images.unsplash.com/photo-1501443762994-82bd5dabb892?q=80&w=400&auto=format&fit=crop'" alt="Produk" class="w-full h-full object-cover">
                                        </div>
                                        <div>
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    <div class="space-y-2">
                                        ...
                                        <input type="number" x-model.number="item.qty" @input="validateQuantity(item)"
                ...
                <div class="p-8 bg-slate-900 text-white space-y-6">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex gap-3">
                            ...
                            <button @click="submitPO()" :disabled="!selectedCustomerId || loading" class="px-8 py-3 bg-brand-blue text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl shadow-brand-blue/20 disabled:opacity-50">
                                <span x-text="loading ? 'Memproses...' : 'Kirim PO'"></span>
                            </button>
                        </div>
                    </div>
                </div>
...
@push('scripts')
            ...
            return {
                ...
                async submitPO() {
                    ...
                    const result = await Swal.fire({
                        ...
                        text: "Kirim Purchase Order ini ke koordinator?",
                        icon: 'question',
                    ...
                    if (result.isConfirmed) {
                        this.loading = true;
                        try {
                            const response = await fetch("{{ route('sales.purchase-order.store') }}", {
                                method: 'POST',
                                headers: {
    ...
    <style>
        ...
        @media (min-width: 1024px) {
            #cartPanel { transform: none !important; position: relative !important; max-height: none !important; }
        }
    </style>
@endpush