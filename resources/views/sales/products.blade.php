    ...
    <div class="p-6">
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Pantau ketersediaan stok dan harga produk secara real-time.</p>
            </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm mb-8">
            <form action="{{ route('sales.products.index') }}" method="GET" class="flex flex-wrap items-center gap-4">
                <!-- Search -->
                <div class="relative group flex-1 min-w-[240px]">
            ...
            @forelse($products as $product)
                <div class="group bg-white rounded-2xl p-4 border border-slate-100 hover:border-brand-blue/30 transition-all duration-300 hover:shadow-xl hover:shadow-brand-blue/5 hover:-translate-y-1">
                    ...
                    <div class="relative aspect-square rounded-xl bg-slate-50 overflow-hidden mb-4">
                        <img src="{{ Storage::url(ltrim($product->image, '/')) }}" 
                            alt="Produk"
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100/50">
                            <div class="flex flex-col">
                                <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Harga Jual</span>
                                <span class="text-sm font-black text-blue-600 tracking-tight">Rp {{ number_format($product->selling_price, 0, ',', '.') }}</span>
                            </div>
                            <div class="h-6 w-[1px] bg-slate-200"></div>
                            <div class="flex flex-col items-end">
                                <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Stok</span>
                                <span class="text-sm font-black text-slate-900 tracking-tight">{{ $product->current_stock }} <span class="text-[10px] text-slate-400">{{ $product->unit }}</span></span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-1">
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">{{ $product->current_stock > 0 ? 'Ready Stock' : 'Stok Habis' }}</span>
                            </div>
                            <span class="text-[10px] font-bold text-slate-300">#{{ str_pad($product->id_product ?? $loop->iteration, 4, '0', STR_PAD_LEFT) }}</span>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-span-full py-20 flex flex-col items-center justify-center bg-white rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <p class="text-sm text-slate-500 mt-1 font-medium">Coba gunakan kata kunci lain atau reset filter.</p>
                    <a href="{{ route('sales.products.index') }}" class="mt-6 px-6 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-900/10">Reset Filter</a>
                </div>
            @endforelse
        </div>
